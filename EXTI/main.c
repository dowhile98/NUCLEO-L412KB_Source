...
void EXTI1_Config(void){
	...
	/*1. configurar el pb1 como entrada*/
	GPIOB->MODER &=~ GPIO_MODER_MODE1; 			//modo entrada
	GPIOB->PUPDR &=~ GPIO_PUPDR_PUPD1;
	GPIOB->MODER |= GPIO_PUPDR_PUPD1_1;			//pull down
	/*2 configurar el detector de borde*/
	EXTI->RTSR1 |= EXTI_RTSR1_RT1;					//rising edge
	/*3. hacer la interrupcion de la linea sea no enmcascarable*/
	EXTI->IMR1 |= EXTI_IMR1_IM1;
	/*4. asignar el pin a la linea exti*/
	SYSCFG->EXTICR[0] &=~ SYSCFG_EXTICR1_EXTI1;
	SYSCFG->EXTICR[0] |= SYSCFG_EXTICR1_EXTI1_PB;
	/*5. identificar el IRQn*/
	//IRQn = 7
	/*6. configurar la prioridad*/
//	NVIC->IP[7] = 3U<<4;
//	NVIC->IP[EXTI1_IRQn] = 3U<<4;
	NVIC_SetPriority(EXTI1_IRQn,3);
	/*7 Habilitar la interrupcion en la NVIC*/
